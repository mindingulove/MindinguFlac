...
def _has_rg() -> bool:
    try:
        ...
        subprocess.run(["rg", "--version"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, startupinfo=si)
        return True
    except Exception:
        return False
...
def _run_rg(query: str, path: str = ".", limit: int = 200) -> List[Dict]:
    ...
    try:
        ...
        if os.name == "nt":
            ...
            si.dwFlags |= subprocess.STARTF_USESHOWWINDOW
            si.wShowWindow = 0 # SW_HIDE
        proc = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True, check=False, startupinfo=si)
