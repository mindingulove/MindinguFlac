...
class AppleMusicProvider(BaseProvider):
    ...
    def _resolve_track_url_from_songlink(self, source_url: str) -> str | None:
        if not source_url or "spotify.com" not in source_url:
            return None
        try:
            api_url = "https://api.song.link/v1-alpha.1/links"
            resp = self._session.get(
                api_url,
                params={"url": source_url, "userCountry": "US"},
                headers={"User-Agent": _DEFAULT_UA},
                timeout=15,
            )
            resp.raise_for_status()
            data = resp.json()
            apple = (data.get("linksByPlatform") or {}).get("appleMusic") or {}
            return apple.get("url") or apple.get("nativeAppUri") or None
        except Exception as e:
            logger.debug("[apple-music] Songlink Apple URL resolution failed: %s", e)
        return None
    ...
    def _get_stream_url(self, track_url: str, codec: str) -> tuple[str | None, str | None]:
        ...
        try:
            ...
            logger.debug("[apple-music] app2 rejected for %s: %s", codec, e)
        ...
        try:
            ...
            while time.time() < deadline:
                ...
                st_resp = self._http.get(f"{API_ENDPOINTS['proxy_queued']}/status/{job_id}")
    ...
    def download_track(
        ...
        if not metadata.isrc and not is_native_apple and not (metadata.title and metadata.artists):
            return DownloadResult.fail(self.name, "No ISRC, title/artist, or Apple Music URL provided for resolution.")
        ...
        try:
            ...
            if is_native_apple:
                ...
                logger.info("[apple-music] Using native Apple URL: %s", track_url)
            else:
                # 1. Songlink (Spotify-to-Apple)
                track_url = self._resolve_track_url_from_songlink(metadata.external_url or "")
                if track_url:
                    logger.info("[apple-music] Resolved via Songlink: %s", track_url)

                # 2. ISRC lookup via iTunes
                if not track_url and metadata.isrc:
                    track_url = self._resolve_track_url(metadata.isrc)
                    if track_url:
                        logger.info("[apple-music] Resolved via ISRC (%s): %s", metadata.isrc, track_url)

                # 3. Textual search fallback
                if not track_url:
                    logger.debug("[apple-music] ISRC/Songlink failed, trying textual search...")
                    ...
                    if track_url:
                        logger.info("[apple-music] Resolved via Search: %s", track_url)

            if not track_url:
                raise TrackNotFoundError(self.name, f"Track not found (ISRC: {metadata.isrc or 'N/A'})")

            stream_url = None
            used_codec = None
            api_used = None
            last_proxy_error = "No proxy response"
            ...
            for current_codec in codecs_to_try:
                ...
                # Se _get_stream_url fallisce, potrebbe restituire (None, None) 
                # ma vogliamo catturare il motivo se possibile.
                if stream_url:
            ...
            if not stream_url or not used_codec:
                # Tentativo finale: Se alac fallisce, prova a forzare aac-legacy come ultima spiaggia 
                # anche se non era nei codecs_to_try (per sicurezza)
                if "aac-legacy" not in codecs_to_try:
                    logger.info("[apple-music] All requested codecs failed, trying aac-legacy as last resort...")
                    api_used, stream_url = self._get_stream_url(track_url, "aac-legacy")
                    if stream_url:
                        used_codec = "aac-legacy"

            if not stream_url or not used_codec:
                # Prova a vedere se il problema è la regione dell'URL
                if "music.apple.com/us/" in track_url:
                    new_url = track_url.replace("/us/", "/gb/") # Prova UK come fallback
                    logger.info("[apple-music] Retrying with UK storefront: %s", new_url)
                    for current_codec in codecs_to_try:
                        api_used, stream_url = self._get_stream_url(new_url, current_codec)
                        if stream_url:
                            used_codec = current_codec
                            track_url = new_url
                            break
            ...
            if not stream_url or not used_codec:
                raise SpotiflacError(ErrorKind.UNAVAILABLE, "No audio stream available (fallbacks exhausted). This usually means the track is region-locked or the proxy is currently down.", self.name)
